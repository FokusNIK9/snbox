...
public sealed class UnitAnimSync : Component
{
	...
	public string GroundedParam { get; set; } = "grounded";

	[Property, Group( "Parameters" ), Description( "Anim parameter name for alive state (bool)" )]
	public string AliveParam { get; set; } = "alive";
	...
	protected override void OnUpdate()
	{
		...
		TargetRenderer.Set( SpeedParam, SyncedSpeed );
		TargetRenderer.Set( GroundedParam, SyncedGrounded );
		TargetRenderer.Set( AliveParam, SyncedAlive );
		TargetRenderer.Set( WishDirParam, SyncedWishDir );
	}
}
